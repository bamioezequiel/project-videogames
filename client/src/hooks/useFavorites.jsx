...
export default function useFavorites() {
    ...
    async function saveAllItemsInFavorites(userId) {
        ...
        for (let i = 0; i < favoritesLS.length; i++) {
            if (favoritesDB?.find((g) => g.id == favoritesLS[i].id)) { continue }
            else {
    ...
    async function setItemFavorites(id) {
        if (isAuth) {
            favoritesDB?.find((c) => c.id == id && setFavorites(true));
        } else {
            let favoritesLS = JSON.parse(localStorage.getItem("favorites") || '[]');
            favoritesLS?.find((c) => c.id == id && setFavorites(true));
        }
    }

    function findItemFavorites(id) {
        let res = false;
        let favoritesLS = JSON.parse(localStorage.getItem("favorites") || '[]');
        favoritesLS?.find((c) => c.id == id && (res = true));
        return res;
    }

    async function handleFavorites(e, game) {
        e.preventDefault();

        if (isAuth) {
            if (favoritesDB?.find((g) => g.id == game.id)) {
            ...
            if (!findItemFavorites(game.id)) {
                if (!localStorage.getItem("favorites")) {
                    ...
                    let favoritesLS = JSON.parse(localStorage.getItem("favorites") || '[]');
                    if (favoritesLS?.filter((c) => c.id !== game.id)) {
                ...
                let favoritesLS = JSON.parse(localStorage.getItem("favorites") || '[]');
                let remFavorites = favoritesLS.filter((c) => {
                    return c.id !== game.id;
                });
